        ...
        <div class="-space-y-px rounded-md shadow-sm">
            ...
            <form action="{{ route('register.custom') }}" method="POST">
                ...
                <div class="mb-8">
                    ...
                    @endphp
                    <select id="name" class="relative block w-full appearance-none  rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" name="level_of_educatio" required>
                        @foreach($lists as $list)
                        <option>{{$list}}</option>
                        @endforeach
                    </select>
                    @if ($errors->has('level_of_eduction'))
                    <span class="text-danger">{{ $errors->first('level_of_eduction') }}</span>
                    @endif
                </div>
                <div class="mb-8">
